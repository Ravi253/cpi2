...
//    Logger log = LoggerFactory.getLogger(this.getClass());
    ...
    try {
        ...
        // processHeadersAndProperties(    "{LOG_ID}_{TIMESTAMP}_{COUNTER}_info",           message); // for adding timestamp to filename
...
def String resolveVariables(String str, Message message) {
  ...
  def timestamp = new java.text.SimpleDateFormat("yyyyMMdd_HHmmss").format(currentDate);
            ...
            if (ex instanceof org.apache.cxf.interceptor.Fault) {
             ...
             exmap.put("getCause",""+ex.getCause());
                ...
                if (ex.getCause()!=null) {
                  ...
                  def String cause_clazz = ex.getCause().getClass().getCanonicalName();
                  exmap.put("getCause.clazz.name",cause_clazz);
                  if (ex.getCause() instanceof org.apache.cxf.transport.http.HTTPException) {
                    cause_message = ex.getCause().getResponseMessage();
                    message.getHeaders().put("cause_message.getResponseMessage", ex.getMessage() +": "+ ex.getCause().getResponseMessage());
                
                  } else if (ex.getCause() instanceof java.net.SocketTimeoutException) {
                    java.net.SocketTimeoutException socketTimeoutException = ex.getCause();
                    cause_message = socketTimeoutException.getMessage();
                  }
...
def Message CompressData(Message message) {
    def map = message.getProperties();
    def body = message.getBody(java.lang.String) as String;
    def CompressedPart = map.get("CompressedPart");	
//    def messageLog = messageLogFactory.getMessageLog(message);
//    if (messageLog != null) {
...
//        }
    ...
    //Parse the xml
     def value = "New Hire"
 def customString25 = "<CompressedPart Type='gzip'>"+CompressedPart+"</CompressedPart>"

  def response= new XmlSlurper().parseText(body)
  def newNode = new XmlSlurper().parseText( customString25 )     
    
	response.GovTalkMessage/Body/IRenvelope/FullPaymentSubmission/CompressedPart.appendNode( newNode );
//  response.EmpEmployment[0].jobInfoNav[0].EmpJob[0].appendNode( newNode );
  String outxml = groovy.xml.XmlUtil.serialize( response )
